...
public:
	...
	template<class... Args> explicit CircularElement<T>(Args&&... args) : m_data(std::forward<Args>(args)...) 
	{
	}
...
template<class T, int MAX_CHILDREN> std::vector<std::array<T, 2>> concaveHull(
...
// Monotone_chain algorithm
// Returns list of index on the convex hull in counter-clockwise order.
// Note: the last index in the returned list is the same as the first one.
template<class T> std::vector<int> convexHull(const std::vector<std::array<T, 2>>& points)
{
	...
	std::sort(sorted.begin(), sorted.end(), [&points](int a, int b) { return points[a][0] == points[b][0] ? points[a][1] < points[b][1] : points[a][0] < points[b][0]; });

	// Construct upper and lower hulls
	std::vector<int> lower = { sorted[0], sorted[1] };
	std::vector<int> upper = { sorted[0], sorted[1] };
	for (int i = 2; i < n; ++i)
	{
		int idx = sorted[i];
		//std::array<T, 2> p{ points[idx][0], points[idx][1] };
		const std::array<T, 2>& p = points[idx];

		// Insert into lower list
		int m = lower.size();
		while (m > 1 && orient2d(points[lower[m - 2]], points[lower[m - 1]], p) <= 0)
		{
			m--;
			lower.pop_back();
		}
		...
		m = upper.size();
		while (m > 1 && orient2d(points[upper[m - 2]], points[upper[m - 1]], p) >= 0)
		{
			m--;
			upper.pop_back();
		}
	...
	result.resize(upper.size() + lower.size() - 2);
	int ptr = 0;
	for (int i = 0, nl = lower.size(); i < nl; ++i)
	{
		result[ptr++] = lower[i];
	}
