...
namespace concaveman
{
    internal class Program
    {
        /*
            ...
            [92, 77, 96, 80], 
            [87, 70, 90, 74],
            [43, 41], [47, 43], [0, 58], [2, 62]
            , 76, 86, 80, 89, 27, 13, 27, 15, 71, 63, 75, 67, 25, 2, 27, 2, 87,
        ...
        static List<int> addRandomBox(int boxSize)
        {
            var rand = new Random();
            var x = rand.Next() * (100 - boxSize);
            var y = rand.Next() * (100 - boxSize);
            var x2 = x + rand.Next() * boxSize;
            var y2 = y + rand.Next() * boxSize;
            return new List<int> { x, y, x2, y2 };
        }

        static void Main()
        {
            var coords = new List<int>(N);
            for (var i = 0; i < N; i++) coords.AddRange(addRandomBox(1));

            var Tboxes100 = new List<int>(K);
            var Tboxes10 = new List<int>(K);
            var Tboxes1 = new List<int>(K);
            for (var i = 0; i < K; i++)
            {
                Tboxes100.AddRange(addRandomBox(100));
                Tboxes10.AddRange(addRandomBox(10));
                Tboxes1.AddRange(addRandomBox(1));
            }
            var boxes100 = Tboxes100.ToArray();
            var boxes10 = Tboxes10.ToArray();
            var boxes1 = Tboxes1.ToArray();

            Stopwatch stopwatch = new Stopwatch();
            Flatbush<double> tree = new Flatbush<double>(N, 16);

            for (int i = 0; i < coords.Count; i+= 4)
            {
                tree.Add(coords[i], coords[i + 1], coords[i + 2], coords[i + 3]);
            }
            ...
            stopwatch.Start();
            for (int i = 0; i < boxes100.Length; i += 4)
            {
                tree.Search(boxes100[i], boxes100[i + 1], boxes100[i + 2], boxes100[i + 3]);
            }
            stopwatch.Stop();
            Console.WriteLine(stopwatch.Elapsed);
            ...
            /*
            stopwatch.Restart();
            for (var i = 0; i < boxes10.Length; i += 4)
            {
                tree.Search(boxes10[i], boxes10[i + 1], boxes10[i + 2], boxes10[i + 3]);
            }
            stopwatch.Stop();
            Console.WriteLine(stopwatch.Elapsed);

            stopwatch.Restart();
            for (var i = 0; i < boxes1.Length; i += 4)
            {
                tree.Search(boxes1[i], boxes1[i + 1], boxes1[i + 2], boxes1[i + 3]);
            }
            stopwatch.Stop();
            Console.WriteLine("boxes1 " + stopwatch.Elapsed);

            stopwatch.Restart();
            for (var i = 0; i < K; i++)
            {
                tree.Neighbors(coords[4 * i], coords[4 * i + 1], 1, out var _);
            }
            stopwatch.Stop();
            Console.WriteLine(stopwatch.Elapsed);
            ...
            for (var i = 0; i < K; i++)
            {
                tree.Neighbors(coords[4 * i], coords[4 * i + 1], 100, out var _);
            }
            stopwatch.Stop();
            Console.WriteLine(stopwatch.Elapsed);
            ...
            for (var i = 0; i < 1; i++)
            {
                tree.Neighbors(coords[4 * i], coords[4 * i + 1], N, out var _);
            }
            stopwatch.Stop();
            Console.WriteLine(stopwatch.Elapsed);
            ...
            */
            ...
            //var b = tree.Neighbors(8, 8, 20, out var results);
